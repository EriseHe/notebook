...
- Independent 300px left sidebar and 300px right outline, with no separating rule at the outline's left edge. Desktop article width remains 720px. Toggling either panel only hides/shows it: its grid space is reserved so the article never moves, resizes, reflows, or changes scroll position. Pages without a sidebar or outline do not reserve space for an unavailable panel. Below 1400px the panels become overlay drawers rather than squeezing the article; on phones the top navigation uses a second toolbar row.
