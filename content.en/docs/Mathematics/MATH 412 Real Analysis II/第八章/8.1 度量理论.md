...
### 1. 几何动机
积分本质上是计算函数下方区域的「体积（volume）」。如何定义这种「体积」会直接改变到积分的性质。为了计算区域的面积，Lebesgue积分定义的逻辑步骤如下：
...
4. 近似值：对每个小矩形，我们计算函数在其上的最大值和最小值，并用这些值乘以小矩形的体积，得到上和与下和。
5. 当上积分等于下积分时，函数被称为可积的（integrable），这个共同值就是积分。
