# 14. 事件视界（Event Horizon）

> **引子｜问题是什么？**  
> 史瓦西几何在半径 $r_s=2GM$ 处出现一系列“奇怪现象”：在那儿静止的钟不再走时、外面观察到从那儿发出的光有**无限红移**、自由下落的粒子对远处观察者而言好像“被冻结”在 $r_s$ 上，同时度规分量 $g_{rr}$ 发散。  
> **这是几何真的“坏掉”了吗？还是坐标在说谎？**  
> 本章用直观与推导并行的方式说明：这些奇怪之处多半是**坐标病理（coordinate pathology）**，而不是**几何病理（geometric pathology）**；同时阐明为何 $r=2GM$ 是**单向因果边界**——事件视界（event horizon）。
...
> [!definition|14.1] 史瓦西半径  
> $r_s=2GM$。当 $r=r_s$ 时出现所谓“视界行为”。

**目标。** 识别并分类 $r=2GM$ 处的“病理”，用可计算量（固有距、固有时）检验其物理性，解释视界内外**时间/空间角色互换**，并指出史瓦西坐标在视界附近为何失败，从而为可跨越视界的坐标（下一章）做铺垫。

---

## 14.2 视界处的“病理目录”（Catalog at $r=2GM$）
...
\end{aligned}
$$
当 $r_e\to 2GM^+$ 时 $\lambda_R/\lambda_e\to\infty$：对外界而言，**从视界发出的光被无限红移**。

把理想“静止钟”放在 $r$ 处（$dr=d\theta=d\phi=0$），有
$$
\begin{aligned}
d\tau^2
=\Bigl(1-\frac{2GM}{r}\Bigr)dt^2 .
...
> [!remark|14.2] 观察者依赖性  
> “冻结”与“无限红移”是相对于**用远处主钟定义的** $t$ 的说法；换坐标后结论会改变。

---

## 14.3 赤道面上的运动方程（Equations of Motion）
...
> [!remark|14.3] 物理图像  
> 因子 $(1-2GM/r)$ 抑制了对外界坐标时间的变化率，导致“冻结”表象。

---

## 14.4 几何病理 vs 坐标病理：用可观测量检验

**思路。** 若几何在 $r=2GM$ 真正“坏掉”，则到达那里的**固有距离**或**固有时**应发散；若有限，则更像是**坐标坏了**。

### 14.4.1 纯径向的固有距离（proper radial distance）
...
### 14.4.2 自由落体到 $r=0$ 的固有时（从静止于 $R$ 出发）
...
> [!theorem|14.4] 检验结论  
> $\Delta s,\Delta\tau$ 均为**有限**，与“几何发散”不符，支持“**坐标病理**”判断。

---

## 14.5 “时间坐标”的失败：远处主钟与 $t$-计实验
...
> [!remark|14.5] 杆件崩塌  
> 没有任何有质量的“桁条”能静止地撑到 $r=2GM$；其底端必被拉入黑洞。这揭示：**不能对 $r=2GM$ 上的事件赋予有意义的 $t$**。
...
> [!theorem|14.6] 未来即 $r=0$  
> 任何在视界内的类时世界线（乃至类光世界线）其**未来端**必在 $r=0$。穿越视界后，终点不可避免。
...
> [!definition|14.7] 事件视界（event horizon）  
> 一条**仅允许运动向内单向通过**的类时/类光可达边界；其存在使得用“远处主钟”定义的时间 $t$ 在该处**失去物理可实现性**。
...
## 14.9 复盘与检核（Interpretation & Checks）
...
## 14.A 符号与公式速查（Notation & Formulae）
...
> [!remark|14.A] 教学建议  
> 课堂上先以观测现象引出“悖论”，随后用式 (14.3)(14.4) 的**有限性**击破“几何奇点”，再以 $t$-计实验和角色互换完成论证，最后自然过渡到“该换坐标了”。
